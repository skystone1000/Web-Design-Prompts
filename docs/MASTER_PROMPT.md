# Master Prompt — Web Design Reference Site

> Copy-paste this entire prompt into any AI builder (Claude, v0.dev, Bolt, Cursor, Lovable, Stitch, Framer AI). It will build a complete reference website that lives-demos every concept from "The Vibe Coder's Web Design Guide" with copyable prompts for re-use.

---

## ROLE & GOAL

You are a senior frontend engineer + design system author. Build a **complete static website using only HTML, CSS, and vanilla JavaScript** (no React, no frameworks, no build step). The site is a **living reference catalog** of 39 web-design techniques. Every technique must (a) be explained, (b) be live-demoed on its own page so the user can interact with it, and (c) expose the exact prompt that produced it inside a "Copy Prompt" code block.

The user must be able to open `index.html` in a browser and navigate everything with zero install.

---

## TECH CONSTRAINTS

- Pure **HTML + CSS + vanilla JS**. No React/Vue/Svelte. No bundlers. No npm.
- External libs allowed **only via CDN script tags**: GSAP, ScrollTrigger, Lenis, SplitType, AOS, Masonry.js — load only on the pages that need them.
- Use **Google Fonts** and/or **Fontshare** via `<link>` tags.
- Use **CSS custom properties** for the global token system (colors, spacing, typography).
- **No images required** — use CSS gradients, SVGs, emoji, or placeholder.co URLs (e.g. `https://placehold.co/800x600/0a0a0a/667eea`) where a photo is implied.
- Mobile responsive (breakpoints: 480px, 768px, 1024px).
- Code must be clean, commented at section boundaries, and copy-pastable.
...
```
/index.html                          ← landing page + master nav to all 6 categories
/assets/
  /css/
    global.css                       ← tokens, reset, typography, layout primitives
    components.css                   ← shared chrome (navbar, prompt-card, footer)
  /js/
    global.js                        ← nav, theme toggle, copy-to-clipboard for prompts
/pages/
  /1-aesthetics/
    index.html                       ← category overview, links to 8 demos
    1.1-glassmorphism.html
    1.2-neumorphism.html
    1.3-brutalism.html
    1.4-minimalism.html
    1.5-bento-grid.html
    1.6-dark-luxury.html
    1.7-retro-y2k.html
    1.8-claymorphism.html
  /2-scroll-animation/
    index.html
    2.1-parallax.html
    2.2-scroll-triggered.html
    2.3-horizontal-scroll.html
    2.4-sticky-pinning.html
    2.5-text-reveal.html
    2.6-scroll-progress.html
    2.7-smooth-scroll-lenis.html
  /3-layouts/
    index.html
    3.1-f-pattern.html
    3.2-hero-feature-grid.html
    3.3-asymmetric-split.html
    3.4-masonry.html
    3.5-fullpage-scroll.html
    3.6-sidebar-content.html
  /4-navigation/
    index.html
    4.1-sticky-navbar.html
    4.2-hamburger-mobile.html
    4.3-mega-menu.html
    4.4-dot-navigation.html
    4.5-breadcrumbs.html
    4.6-tab-navigation.html
  /5-typography-color/
    index.html
    5.1-type-scale.html
    5.2-font-pairing.html
    5.3-60-30-10-rule.html
    5.4-gradient-text.html
    5.5-display-hero-type.html
    5.6-dark-light-toggle.html
  /6-ui-patterns/
    index.html
    6.1-hero-section.html
    6.2-pricing-table.html
    6.3-testimonials.html
    6.4-accordion-faq.html
    6.5-modal-dialog.html
    6.6-card-component.html
```

---

## DEMO PAGE TEMPLATE (use for ALL 39 sub-pages)

Each individual demo page must follow this exact section order:

1. **Sticky top navbar** (shared component) — logo "Vibe Design Ref", breadcrumb-style trail "Aesthetics › Glassmorphism", a dark/light toggle.
2. **Hero band** — chapter number badge ("1.1"), title ("Glassmorphism"), one-sentence tagline.
3. **"What it is" card** — the plain-English explanation, verbatim from the source guide below.
4. **"How it works" card** — the mechanism explanation, verbatim from the source guide below. Below it, a small key-terms grid (term + 1-line definition).
5. **🔴 LIVE DEMO section** — full-bleed `<section>` that actually renders the effect using the AI prompt's specs. This is the most important part. It must visibly demonstrate the technique. Add a small "Demo" badge in the top-right corner of this section.
6. **"Copy this prompt" card** — a `<pre><code>` block containing the EXACT prompt text from the guide (see the data below), with a **Copy** button that copies the prompt to clipboard via `navigator.clipboard.writeText()`. Show a toast/check-mark on success.
7. **TIP callout** (only if the guide has a TIP for this entry — included in data below).
8. **"Example sites to study" list** — bulleted list of the 3-4 reference sites with names, one-line descriptions, and external links (open in new tab).
9. **Prev / Next pager** at the bottom — links to the previous and next demo in the chapter order.
10. **Footer** — © + small "Built from The Vibe Coder's Web Design Guide".

---

## GLOBAL DESIGN SYSTEM

The shell of the site itself is **dark-luxury minimal** (so individual demo pages can showcase contrasting aesthetics without competing).
...
- **Font pairing for the shell**: Headings = `Space Grotesk` (700/800). Body = `Inter` (400/500). Code = `JetBrains Mono`.
- **Heading scale**: H1 `clamp(40px, 6vw, 72px)`; H2 `clamp(28px, 3.5vw, 44px)`; H3 `22px`; body `17px / line-height 1.65`.
- **Page width**: content capped at 1100px, with the LIVE DEMO section allowed to break out full-bleed.

### Shared components to build once and reuse

- `.navbar` — sticky, frosted glass when scrolled past 40px.
- `.prompt-card` — dark card with copy button, language label "Prompt", monospace.
- `.term-grid` — 2-col responsive grid for key-term definitions.
- `.tip` — left-border accent callout with ⚡ icon.
- `.pager` — flex row with prev/next links and arrows.
...
- Hero: title "The Vibe Coder's Web Design Reference", subtitle "39 live demos. 39 copy-paste prompts.", two CTAs: "Browse Aesthetics" → /pages/1-aesthetics/, "Quick Reference Vocabulary" → anchor.
- 6 large category cards (bento-grid style) linking to each `/pages/N-*/index.html`. Each card shows number, name, count of demos, and a tiny visual preview.
- Final section: "Quick Reference: Prompting Vocabulary" — render the table at the end of the source guide (see data) as a 2-column grid.
- Footer.

### Category `/pages/N-*/index.html` pages

- Hero with category number, name, and intro paragraph from the guide.
- Grid of demo cards (3 columns desktop, 1 mobile) — each card: number, title, one-line tease, "View demo →" link.
- Pager to previous/next category at the bottom.

---

## LIVE DEMO REQUIREMENTS (the critical part)

The LIVE DEMO section on each sub-page must follow the technique's prompt **as if you were the AI being prompted**. Concretely:

- **1.1 Glassmorphism**: Render the 3 frosted-glass cards over a `#667eea → #764ba2` gradient with the navbar glass strip. Use `backdrop-filter: blur(12px)`.
- **1.2 Neumorphism**: Render the music player with dual box-shadows on `#e0e5ec`, including the pressed state on `:active`.
- **1.3 Brutalism**: Render the brutalist portfolio fragment — 3px black borders, `#FFE600` yellow accents, slight `rotate(-1deg)` on cards, monospace body.
- **1.4 Minimalism**: Render the photography-portfolio fragment on `#FAFAF9` with Cormorant Garamond / Playfair Display.
- **1.5 Bento Grid**: Build the 3-col bento with 1 large card, 1 tall card, 4 small cards on `#0A0A0A`.
- **1.6 Dark Luxury**: Render the dark SaaS hero block with the radial purple glow + `#0066FF` CTA glow.
- **1.7 Retro/Y2K**: Render the Y2K hero with hot-pink/cyan/lime, sparkle ✦ decorations, chrome gradient text.
- **1.8 Claymorphism**: Render claymorphic task cards + button with `translateY` press animation.
- **2.1 Parallax**: Implement 3-layer parallax on scroll using `requestAnimationFrame` + `translateY`.
- **2.2 Scroll-triggered**: Build the feature grid with IntersectionObserver fade-up + 0.1s stagger.
- **2.3 Horizontal scroll**: Use GSAP ScrollTrigger to pin a section and scrub 5 cards horizontally.
- **2.4 Sticky pinning**: Build the 40/60 layout with `position: sticky` left visual + scrolling feature blocks that swap the left screenshot via IntersectionObserver.
- **2.5 Text reveal**: Split a hero headline into word spans and stagger their fade-up on load.
- **2.6 Scroll progress**: Add a top progress bar (`position: fixed; transform: scaleX(progress)`) on a long-form mock article.
- **2.7 Smooth scroll (Lenis)**: Initialize Lenis via CDN and demonstrate buttery scroll on a tall page.
- **3.1 F-pattern**: Render the article + sticky right ToC layout.
- **3.2 Hero + feature grid**: Render the SaaS landing template (hero, logo strip, 3×2 feature grid, testimonial, mini pricing, CTA).
- **3.3 Asymmetric split**: Render two 60/40 split sections, alternating sides.
- **3.4 Masonry**: Render a 3-col CSS-columns masonry of varied-height tiles.
- **3.5 Fullpage scroll**: Use `scroll-snap-type: y mandatory` on a container with 5 `100vh` sections and right-side dot nav.
- **3.6 Sidebar + content**: Render the 240px dark sidebar + dashboard content with 4 stat cards and a table.
- **4.1 Sticky navbar**: Transparent at top → frosted glass after 60px scroll.
- **4.2 Hamburger**: 3-line icon → animated X, fullscreen overlay menu with staggered link reveal.
- **4.3 Mega menu**: 4-col mega panel on hover of "Women" link (clothing / accessories / occasion / featured image).
- **4.4 Dot navigation**: Vertical dots on the right of a 6-section fullpage scroll, with tooltips and IntersectionObserver active state.
- **4.5 Breadcrumbs**: Render `Home › Men's Clothing › Outerwear › Winter Jackets` with `›` separators via `::before`.
- **4.6 Tab navigation**: 4 tabs (Overview / Specs / Reviews / FAQ) with a sliding 2px blue underline indicator that translates between tabs.
- **5.1 Type scale**: Render the full clamp() scale (display → small) as a live specimen with size labels.
- **5.2 Font pairing**: Load Clash Display + Satoshi from Fontshare and render a sample article showing the pairing.
- **5.3 60-30-10 rule**: Render a mini brochure layout demonstrating the 60% off-white / 30% warm-gray / 10% electric-blue distribution, with a percentage legend.
- **5.4 Gradient text**: Render a hero with one gradient word ("convert") and a second gradient word in a sub-line.
- **5.5 Display typography**: Render the typographic-hero with italic + gradient on "digital experiences".
- **5.6 Dark/light toggle**: Real working toggle that swaps `data-theme` on `<html>`, persists to `localStorage`, respects `prefers-color-scheme`.
- **6.1 Hero section**: Build the productivity-SaaS hero with badge pill, gradient word, two CTAs, trust row, browser-mockup screenshot.
- **6.2 Pricing table**: 3-tier pricing with monthly/annual toggle that animates prices.
- **6.3 Testimonials**: Dual-row marquee scrolling in opposite directions via CSS keyframes.
- **6.4 Accordion FAQ**: 8 questions with rotating + icon, single-open behavior, search filter input.
- **6.5 Modal**: Trigger button → animated scale+fade modal with sign-up form, Escape-to-close, click-overlay-to-close, body-scroll-lock.
- **6.6 Card component**: 3-col blog card grid with hover-lift, image aspect-ratio, line-clamped title/excerpt, meta row.

> **If a CDN library is needed (GSAP / Lenis / SplitType / Masonry), include the `<script>` tag at the top of that page only.**

---

## CONTENT DATA — paste these strings verbatim into each page

Below is the full content for every sub-page. Format per entry:
- `WHAT_IT_IS:` — paste into the "What it is" card
- `HOW_IT_WORKS:` — paste into the "How it works" card
- `KEY_TERMS:` — render as the term-grid (each `term — definition`)
- `PROMPT:` — paste verbatim into the copy-prompt code block
- `TIP:` — render in the tip callout (if present)
- `EXAMPLES:` — render as the example-sites list

---

### 1. Design Aesthetics

> **Category intro**: Design aesthetics are the visual language of a website — the overall "vibe" and personality. When you look at a website and think "that looks futuristic" or "that feels premium", you're reacting to its aesthetic. Understanding these styles lets you describe exactly what you want to an AI.

#### 1.1 Glassmorphism
**WHAT_IT_IS:** Glassmorphism makes UI elements look like frosted glass — semi-transparent, blurred, with a subtle border. Imagine a glass panel sitting in front of a colorful, blurry background. The panel lets some of the background bleed through while remaining readable.
**HOW_IT_WORKS:** The magic happens with two CSS properties: `backdrop-filter:blur()` which blurs everything behind the element, and a `background-color` set to `rgba()` (red, green, blue, alpha) where alpha is a low value like 0.15 making it partially transparent. A thin white border at low opacity completes the glass look. You always need a colorful, vibrant background behind the glass panels — otherwise there's nothing to blur.
**KEY_TERMS:**
...
**EXAMPLES:**
- Awwwards Minimalist Category — awwwards.com/websites/minimal
- Linear.app — One of the cleanest, most minimal SaaS designs — linear.app
- Typewolf.com — Typography-focused minimal design inspiration — typewolf.com

#### 1.5 Bento Grid
**WHAT_IT_IS:** A bento grid is a layout (and aesthetic) inspired by a Japanese bento box — a grid of varied-size compartments, each showing a different feature or piece of content. Popularized by Apple's Mac product pages, it's now one of the most popular design trends for SaaS and product landing pages. Think of it as a mosaic of cards, each with its own design, creating a visual collage.
**HOW_IT_WORKS:** CSS Grid is the technical foundation. Some cards span 2 columns, others are tall and span 2 rows, some are small and square. Cards have different visual treatments: some dark (#111), some light (#F5F5F5), some with gradient backgrounds, some with product screenshots, some with large typography and a stat. The variety in size AND color is what makes it feel premium.
**KEY_TERMS:**
...
**EXAMPLES:**
- Apple Mac product page — The gold standard of bento grids — apple.com/mac
- Linear.app homepage — Dark bento-style feature grid — linear.app
- Framer.com — Beautiful bento grid on their homepage — framer.com
- Dribbble — Bento Grid UI — dribbble.com/tags/bento

#### 1.6 Dark Mode / Dark Luxury
**WHAT_IT_IS:** Dark luxury uses deep charcoal or near-black backgrounds to create a premium, sophisticated atmosphere. Think of a high-end watch brand website or a premium SaaS product. It's not just dark mode — it's a deliberate aesthetic choice that uses darkness to make accent colors and product screenshots pop dramatically.
**HOW_IT_WORKS:** Background is near-black (#0A0A0A or #0D0D1A), NOT pure black (#000000) which can feel harsh. Cards and sections use slightly lighter darks (#111111, #1A1A2E) to create subtle depth. A single accent color — often electric blue (#0066FF), amber (#F59E0B), or soft purple (#8B5CF6) — is used sparingly for CTAs, icons, and highlights. Typography is pure white or near-white (#F1F1EE). Thin borders at low opacity (rgba(255,255,255,0.08)) separate sections softly.
**KEY_TERMS:**
- Near-black — #0A0A0A or #0D0D1A — not pure black
- Accent color — One bright color used ONLY for important elements
- Border opacity — Very low opacity borders like rgba(255,255,255,0.08) for subtle separation
- Subtle gradient — A very faint radial gradient in the background gives depth
...
**EXAMPLES:**
- Firewatch Game Website — firewatchgame.com
- Webflow University Parallax Tutorial — university.webflow.com
- Awwwards Parallax Sites — awwwards.com/websites/parallax

#### 2.2 Scroll-Triggered Animations
**WHAT_IT_IS:** Elements are hidden (invisible and/or off-position) when the page loads. As the user scrolls down and a section enters the visible area of the screen (the "viewport"), that element animates into place. This is by far the most common animation technique on modern websites. The classic version is "fade up" — element starts invisible and slightly below its final position, then fades in and slides up.
**HOW_IT_WORKS:** The IntersectionObserver JavaScript API watches when elements enter or leave the viewport. When an element enters, a CSS class is added to it (like `.visible`). That class triggers a CSS transition or animation that plays the reveal. The element starts with `opacity:0` and `transform:translateY(40px)`, then transitions to `opacity:1` and `translateY(0)`. The timing — how long the animation takes and its easing — determines whether it feels smooth or janky.
**KEY_TERMS:**
- IntersectionObserver — A JavaScript API that fires a callback when an element enters/leaves the viewport
- opacity: 0 → 1 — Element goes from invisible to fully visible
- translateY(40px) → 0 — Element moves upward as it reveals
- Stagger — Delaying each element slightly more than the previous
- AOS library — Animate On Scroll — a simple JavaScript library
...
**EXAMPLES:**
- Resn.co.nz — resn.co.nz
- SplitType.js Demo — split-type.vercel.app
- Awwwards Typography — awwwards.com/websites/typography

#### 2.6 Scroll Progress Indicator
**WHAT_IT_IS:** A thin bar, usually at the very top of the page, that fills from left to right as the user reads down the page. When the user is at the top, the bar is empty. When they reach the bottom, the bar is full. It gives readers a sense of how far through the content they are, particularly useful on long blog posts and articles.
**HOW_IT_WORKS:** A fixed-position element sits at the top of the viewport (`position:fixed, top:0, left:0, height:3-4px`). Its width is calculated as a percentage: `(scrollY / (document.body.scrollHeight - window.innerHeight)) * 100`. This formula gives 0% when at the top and 100% when fully scrolled. A JavaScript scroll event listener updates the width in real-time. Using CSS `transform:scaleX()` instead of changing width is smoother on most devices.
**PROMPT:** Add a reading progress bar to a blog post page. Create a fixed element: position:fixed, top:0, left:0, height:3px, width:100%, z-index:9999. Inside it, a div that starts at transform:scaleX(0) and transform-origin:left. On scroll event, calculate progress: scrolled / (totalHeight - viewportHeight). Update the inner div's scaleX to the progress value (0 to 1). Use a gradient fill from purple #667eea to pink #f64f59. Add a smooth transition of 0.1s on the scale for a slight lag that feels natural.
**EXAMPLES:**
- Medium.com articles — medium.com
- CSS Tricks article on progress bars — css-tricks.com
- Smashing Magazine — smashingmagazine.com

#### 2.7 Smooth Scroll / Lenis
**WHAT_IT_IS:** By default, browsers scroll in a very mechanical, instant way — you flick the wheel and the page jumps. Smooth scroll replaces this with a physics-based momentum system: when you scroll, the page decelerates smoothly instead of stopping abruptly, like a heavy object slowly coming to rest. Lenis is the most popular modern JavaScript library for this. When you visit a website and think "this scroll feels incredible, so buttery" — that's almost always Lenis.
**HOW_IT_WORKS:** Lenis works by overriding the browser's native scroll. It intercepts scroll input (mouse wheel, touch swipe, trackpad), applies its own easing and momentum math, and then moves the page via CSS transform instead of actual scrolling. This is smoother because CSS transforms are GPU-accelerated. Lenis is initialized with a few lines of JavaScript and runs on every animation frame using `requestAnimationFrame`. It integrates directly with GSAP ScrollTrigger.
**KEY_TERMS:**
- Lenis — The most popular smooth scroll JavaScript library — free and widely used
- requestAnimationFrame — Fires a callback 60 times per second
- Easing — The math curve controlling deceleration
- lerp — Linear interpolation
- GSAP compatibility — Lenis passes its scroll position to GSAP
**TIP:** Smooth scroll is a FEEL upgrade, not a visual one. Users might not notice it consciously, but the site feels premium and polished. However: never apply smooth scroll to pages with fixed-height modals or overflow:hidden sections — it can cause visual glitches.
**PROMPT:** Add Lenis smooth scrolling to this website. Import Lenis from CDN: <script src='https://unpkg.com/lenis@1.1.13/dist/lenis.min.js'></script>. Initialize with: const lenis = new Lenis({ duration: 1.2, easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), smooth: true }). Run in animation loop: function raf(time) { lenis.raf(time); requestAnimationFrame(raf); } requestAnimationFrame(raf). If GSAP ScrollTrigger is used on this page, also add: lenis.on('scroll', ScrollTrigger.update) and gsap.ticker.add((time) => { lenis.raf(time * 1000) }).
**EXAMPLES:**
- Lenis by Studio Freight Demo — lenis.darkroom.engineering
- Awwwards Sites using Lenis — awwwards.com
...
> **Category intro**: Layout is how you organize information on the page — where things go and how they relate to each other. A great layout guides the reader's eye from the most important element to the least important, without them noticing.

#### 3.1 F-Pattern Layout
**WHAT_IT_IS:** Eye-tracking research has shown that when people read web pages, their eyes don't scan every word equally. Instead, they trace an F-shape: first they scan across the very top of the page, then they scan a shorter horizontal band a little lower, then their eyes drift down the left edge of the page reading only the first few words of each line. Understanding this pattern helps you place your most important content where eyes naturally fall.
**HOW_IT_WORKS:** The practical takeaway is: put your most important content in the top horizontal band (the headline, navigation, hero message). Put your second most important element below that but still near the top-left. Your left margin and left edge of content are seen far more than the right. Content on the far right of the page is often never seen at all by users who are scanning rather than reading.
**KEY_TERMS:**
- Above the fold — The top portion visible without scrolling
- Left edge priority — Left column and left-aligned text gets far more attention
- Visual anchors — Bold text, images, and icons create stopping points
- Z-pattern — For pages with less text, eyes scan in a Z rather than F
- Scanning vs reading — Most users scan first, read only if interested
**TIP:** The F-pattern is strongest on text-heavy pages. On landing pages with lots of imagery, a Z-pattern (top-left → top-right → bottom-left → bottom-right diagonal) is more common.
...
**EXAMPLES:**
- Medium.com Article layout — medium.com
- Smashing Magazine — smashingmagazine.com
- Nielsen Norman Group F-Pattern research — nngroup.com/articles/f-shaped-pattern-reading-web-content

#### 3.2 Hero + Feature Grid
**WHAT_IT_IS:** The single most common layout for SaaS and product websites. At the top: a large, bold Hero section that states the main value proposition with a CTA button. Below: a grid of Feature cards (usually 3 columns, 2-3 rows) each explaining one capability of the product. This layout has been battle-tested and converts extremely well.
**HOW_IT_WORKS:** Hero section: full-width, center-aligned, bold H1 headline (max 8 words), one-sentence subtext, and a primary CTA button. Below this, often a strip of company logos. Then the feature grid: 3 equal columns, each card with an icon, a 3-5 word title, and a 1-2 sentence description. The grid can have 2-4 rows of cards depending on how many features you're highlighting.
...
- Tailwind UI Components — tailwindui.com/components
- Landingfolio.com — landingfolio.com

#### 3.3 Asymmetric / Split Layout
**WHAT_IT_IS:** Instead of dividing the page into equal 50/50 halves, asymmetric layouts use unequal proportions — like 60/40, 65/35, or 70/30. One side gets the dominant visual (large image, video, illustration), while the other side gets supporting content (text, stats, CTA). The imbalance creates visual tension that makes the layout feel dynamic and intentional.
**HOW_IT_WORKS:** Using CSS Grid with non-equal column definitions: `grid-template-columns: 3fr 2fr` (60/40) or `2fr 1fr` (67/33). The larger column holds the hero image or product screenshot. The smaller column holds the headline, description, and CTA. On mobile, both columns stack to 100% width.
...
**EXAMPLES:**
- FullPage.js Examples — alvarotrigo.com/fullPage/examples
- Webflow Full Page Templates — webflow.com/templates
- Sony BE MOVED campaign — sony.com

#### 3.6 Sidebar + Content Layout
**WHAT_IT_IS:** A persistent navigation panel on the left side of the screen, with the main working area on the right. This is the universal layout for web applications, dashboards, and admin interfaces. The sidebar stays fixed as the main content area scrolls. The sidebar typically contains the app's main navigation, while the content area shows whatever page is currently selected.
**HOW_IT_WORKS:** The outer container uses `display:flex` or CSS Grid with two columns: a fixed-width sidebar (240-280px) and a flex-1 or fr content area that fills the remaining space. The sidebar gets `position:sticky top:0` with `height:100vh` so it stays visible while the content scrolls. The sidebar contains the logo at top, navigation links in the middle (with icons), and user profile at the bottom.
...
- Tailwind UI Application UI — tailwindui.com/components#application-ui
- AdminLTE Demo — adminlte.io

---

### 4. Navigation

> **Category intro**: Navigation is how users find their way around your website. Good navigation is invisible — users don't notice it because they always know where they are and how to get where they want.

#### 4.1 Sticky Navbar
**WHAT_IT_IS:** A navigation bar that stays fixed to the top of the viewport as the user scrolls. Unlike a static nav that scrolls away with the page, a sticky nav is always accessible no matter how far down the user has scrolled. This is the most common and expected navigation pattern for marketing websites and landing pages.
**HOW_IT_WORKS:** `position:sticky top:0` keeps the navbar at the top of the viewport when within its parent. A popular enhancement: the navbar starts transparent (showing the hero content behind it), and as the user scrolls even a few pixels, it transitions to a solid or frosted-glass background. This is detected by listening to the scroll event and checking if `window.scrollY > 0` (or a threshold like 80).
...
**WHAT_IT_IS:** On mobile screens, there isn't room for all navigation links. The hamburger menu (named after its resemblance to a hamburger when viewed from the side) hides all navigation behind a single icon. Tapping it reveals the full navigation — either as a fullscreen overlay, a side drawer, or a dropdown panel.
**HOW_IT_WORKS:** Three horizontal div elements (or a custom SVG) form the icon. When clicked, JavaScript toggles a class on a fullscreen overlay element. The three lines animate into an X using CSS transforms: the top line rotates 45 degrees, the middle line fades to opacity:0, the bottom line rotates -45 degrees. The overlay menu typically appears with a fade or slide-in transition.
...
**EXAMPLES:**
- Awwwards Portfolio Sites — awwwards.com/websites/portfolio
- Codrops Menu Animations — tympanus.net/codrops
- Cuberto.com — cuberto.com

#### 4.3 Mega Menu
**WHAT_IT_IS:** A mega menu is a large dropdown panel that appears when you hover over (or click) a navigation item. Unlike a simple dropdown with a list of links, a mega menu can contain multiple columns of links, featured images, highlighted promotions, icons, and even mini descriptions. They're used on complex websites where a single nav item leads to many sub-pages.
**HOW_IT_WORKS:** When the user hovers a navigation link (or clicks on mobile), an absolutely-positioned full-width or wide panel appears below the nav bar. This panel is typically organized into 3-5 columns, each containing a category heading and 4-8 links below it. One column might be a "Featured" panel with an image and a highlighted promotion. The panel appears with a subtle fade or slide-in animation.
**KEY_TERMS:**
- position:absolute — The mega menu panel is absolutely positioned below the nav bar
- Hover trigger — Desktop: hover. Mobile: tap/click
- Multi-column grid — Links organized into 3-5 equal columns using CSS Grid
- Featured column — One column with an image and CTA
- z-index — Must have high z-index (like 9000) to appear above all page content
...
**EXAMPLES:**
- ASOS.com — asos.com
- Apple.com navigation — apple.com
- Shopify Polaris Navigation docs — polaris.shopify.com

#### 4.4 Dot Navigation
**WHAT_IT_IS:** A small set of dots (usually 4-8) positioned on the right or bottom side of the screen. Each dot represents one section or slide of the website. The dot corresponding to the currently-visible section is filled or larger than the others. Clicking a dot instantly jumps the page to that section. This pattern is almost exclusively used on fullpage scroll websites and slideshow-style presentations.
**HOW_IT_WORKS:** The dots are small circular elements (8-12px diameter) positioned with `position:fixed` on the right edge of the viewport. They're styled with a semi-transparent border by default and filled solid when active. As the user scrolls, JavaScript detects which section is currently in view (using IntersectionObserver) and updates which dot has the 'active' class. Clicking a dot triggers a smooth scroll to that section.
**KEY_TERMS:**
- position:fixed right — Dots stay visible as sections change
- Active state — Larger, fully filled
- IntersectionObserver — Detects which section is currently most visible
- Tooltip on hover — Hovering a dot shows the section name
- scrollIntoView() — JavaScript method to smoothly scroll to an element
**PROMPT:** Add dot navigation to a 6-section fullpage scroll website. Create 6 dots using position:fixed, right:24px, top:50%, transform:translateY(-50%), displayed as a vertical column with 16px gap between dots. Default dot style: 8px width and height, border-radius:50%, border:2px solid rgba(255,255,255,0.6), background:transparent, cursor:pointer, transition all 0.3s. Active dot style: 12px width and height, background:white, border-color:white, box-shadow:0 0 8px rgba(255,255,255,0.5). On hover of any dot, show a tooltip to the left with the section name (absolute positioned, background:rgba(0,0,0,0.7), white text, padding:4px 10px, border-radius:4px). Use IntersectionObserver (threshold:0.5) to update active dot as sections scroll into view.
**EXAMPLES:**
- FullPage.js examples — alvarotrigo.com/fullPage/examples
- Apple Special event pages — apple.com/apple-events
- Dribbble Dot Navigation UI — dribbble.com/tags/dot-navigation

#### 4.5 Breadcrumbs
**WHAT_IT_IS:** Breadcrumbs are a secondary navigation element showing the user's current location within the website's hierarchy. Named after the Hansel and Gretel fairy tale, they appear as a horizontal trail of links: Home › Category › Subcategory › Current Page. They let users understand exactly where they are and jump back to any parent level with one click.
**HOW_IT_WORKS:** Breadcrumbs are an ordered list (HTML `ol` element) of links separated by a visual divider — usually a forward slash (/), chevron (›), or angle bracket (>). The current page is the last item and is NOT a link. All items before it are clickable links. The visual style is intentionally subtle — small font size (13-14px), muted color — so it doesn't compete with the main page heading.
**KEY_TERMS:**
- ol element — Breadcrumbs use an ordered list for correct HTML semantics
- aria-label:breadcrumb — Accessibility attribute for screen readers
- aria-current:page — Marks the current (last) item
- schema.org markup — Makes breadcrumbs appear in Google search results
- Separator — Usually CSS ::after content: '/' or '›'
**PROMPT:** Add a breadcrumb navigation to a product detail page. HTML structure: <nav aria-label='Breadcrumb'><ol> with li items separated by CSS-generated dividers. Style: font-size:13px, color:#888888. Each link (except last): color:#888, text-decoration:none, hover:color:#111 with 0.2s transition. Last item (current page): color:#111, font-weight:500, no link. Separator: CSS li + li::before { content: '›'; margin: 0 8px; color:#CCCCCC }. The breadcrumb path for this page: Home → Men's Clothing → Outerwear → Winter Jackets. Wrap in a container with 16px top and bottom padding. Also add JSON-LD schema.org BreadcrumbList markup in the head for SEO.
**EXAMPLES:**
- Amazon.com product pages — amazon.com
- MDN Web Docs — developer.mozilla.org
- Zalando.com — zalando.com

#### 4.6 Tab Navigation
**WHAT_IT_IS:** Tabs are a horizontal row of clickable labels at the top of a content area, where clicking a tab swaps the content panel below it. Think of a physical filing cabinet with tabbed dividers. Tabs are perfect for organizing related content that users need to switch between without leaving the page.
**HOW_IT_WORKS:** A row of button or anchor elements acts as the tab strip. The currently active tab is indicated visually: an underline `border-bottom` in the brand color, a filled background, bold text, or a combination. Clicking a tab adds an 'active' class to that tab and shows the corresponding content panel while hiding all others. Each tab and panel pair needs `aria-selected`, `role:tab`, `aria-controls` and `role:tabpanel` attributes for accessibility.
**KEY_TERMS:**
- Active indicator — Underline, filled background, or bold text showing the selected tab
- Panel swap — Other panels set to display:none or opacity:0 when inactive
- role:tab / tabpanel — Accessibility attributes
- Keyboard navigation — Arrow keys should move between tabs
- Animated indicator — A sliding underline that moves smoothly between tabs
**TIP:** The sliding animated tab indicator is a great detail. Achieve it with a separate underline element (position:absolute, bottom:0) that moves via transform:translateX() when tabs are clicked. Calculate the target X position from the clicked tab's offsetLeft.
...
**KEY_TERMS:**
- clamp(min, preferred, max) — Font size fluidly between min and max
- Font weight — 100=thin, 400=normal, 600=semibold, 700=bold, 900=black
- Letter-spacing — Negative values on large headings makes them feel tighter
...
- Typewolf.com — typewolf.com

#### 5.2 Font Pairing
**WHAT_IT_IS:** Using two complementary fonts together — one for headings and display text, one for body text and UI. The heading font should have strong personality and visual impact. The body font should be highly readable at small sizes and in paragraphs. The two fonts should contrast but not clash. Using more than two fonts on a website almost always looks amateur.
**HOW_IT_WORKS:** Recommended pairings for different vibes:
- Editorial / Luxury: Playfair Display (headings) + DM Sans (body)
- Modern Startup: Clash Display (headings) + Satoshi (body)
- Tech / SaaS: Syne (headings) + Inter (body)
- Experimental: Monument Extended (headings) + Neue Haas Grotesk (body)
- Friendly / Consumer: Nunito or Poppins (headings) + Source Sans Pro (body)
...
- Tints & shades — Lighter/darker versions of one color
...
**WHAT_IT_IS:** Display typography means using text itself as the primary visual element of a design — the headline is so large, so bold, and so beautifully set that no hero image is needed. At 80px, 100px, or even larger, a single headline becomes a visual experience. This approach is popular on portfolio sites, creative agencies, editorial brands, and tech products that want to lead with confidence and personality rather than a generic stock photo.
**HOW_IT_WORKS:** The key CSS property is `font-size` set very large — typically using `clamp(60px, 9vw, 120px)` so it scales smoothly. Font-weight is usually 700 (bold) or 900 (black) for maximum visual weight. Letter-spacing is tightened with a negative value (-0.03em to -0.05em). Line-height is reduced (0.9 to 1.1) because display text wraps to fewer lines and tight line height creates a compact, powerful block.
**KEY_TERMS:**
- clamp(min, fluid, max) — Scales the font between min and max based on viewport
- Negative letter-spacing — -0.03em to -0.05em on large headings
- Line-height < 1 — 0.9 to 1.05 for display text
- Font-weight 900 — Black or Extra Bold weight
- Text wrapping — Let display headlines wrap naturally to 2-3 lines
...
**EXAMPLES:**
- Awwwards Typography category — awwwards.com/websites/typography
- Obys Agency — obys.agency
- Semplice portfolio builder — semplice.com/inspiration

#### 5.6 Dark / Light Mode Toggle
**WHAT_IT_IS:** A toggle that lets users switch between two complete visual themes — a light version (white/off-white background, dark text) and a dark version (near-black background, light text). Many users prefer dark mode for night-time reading, reduced eye strain, or aesthetic preference. Offering both modes shows attention to user needs and is now considered a standard feature on quality websites.
**HOW_IT_WORKS:** The entire color system is built using CSS custom properties (variables) defined on the `:root` element. Two sets of values are defined — one for light mode, one for dark mode. Switching modes means updating the `data-theme` attribute on the HTML element, which triggers a different set of CSS variable values. JavaScript saves the user's preference to `localStorage` so it persists between visits. The `prefers-color-scheme` media query lets you set the default based on the user's OS setting.
**KEY_TERMS:**
- CSS custom properties — Variables that change value when theme switches
- data-theme attribute — The selector that triggers dark mode CSS values
- prefers-color-scheme — CSS media query detecting OS theme setting
- localStorage — Browser storage that saves the user's theme choice
- Transition — Smooth fade between modes, not instant jump
...
**EXAMPLES:**
- Tailwind UI Pricing Sections — tailwindui.com/components/marketing/sections/pricing
- Pricingpages.xyz — pricingpages.xyz
- Linear.app/pricing — linear.app/pricing

#### 6.3 Testimonials / Social Proof
**WHAT_IT_IS:** A section showing quotes from real users of your product. Social proof is one of the most powerful conversion elements on a website — people trust other people's opinions more than they trust the company selling the product. The best testimonials are specific ("increased our revenue by 40%") rather than generic.
**HOW_IT_WORKS:** Can be laid out as: a 3-column grid of cards, a single large featured quote, or an auto-scrolling marquee (infinite horizontal scroll). The marquee version is very popular currently — cards scroll continuously from right to left, often with two rows moving in opposite directions. Clicking stops the scroll. Star ratings above the quote add credibility.
...
- Stripe.com Customers page — stripe.com/customers

#### 6.4 Accordion / FAQ
**WHAT_IT_IS:** A list of questions where clicking any question expands it to reveal the answer, then clicking again collapses it. This pattern is perfect for FAQ sections, help documentation, and any situation where you have many pieces of related content that would be overwhelming if all shown at once.
**HOW_IT_WORKS:** Each item has a visible question row (with a + or chevron icon on the right) and a hidden answer panel below it. The answer panel starts at `max-height:0` and `overflow:hidden`. When the question is clicked, a class is toggled that changes max-height to a large value (or auto) and the panel reveals with a smooth CSS transition. HTML also has a native `details/summary` element that does this without JavaScript.
...
**EXAMPLES:**
- Webflow University FAQ Templates — university.webflow.com
- Headless UI (Tailwind Labs) — headlessui.com
- Linear.app/pricing FAQ — linear.app/pricing

#### 6.5 Modal / Dialog
**WHAT_IT_IS:** A modal (or dialog) is a popup overlay that appears over the page content, requiring the user to interact with it or dismiss it before returning to the main page. It's used for: confirmations, forms (sign up, log in), image lightboxes, video players, and onboarding flows. The background is darkened to direct focus to the modal.
**HOW_IT_WORKS:** An overlay div covers the entire screen (`position:fixed, inset:0, background:rgba(0,0,0,0.5)`). The modal panel sits centered on top of the overlay, usually achieved with `display:flex, align-items:center, justify-content:center` on the overlay. The modal panel itself is a white card with padding, shadow, and border-radius. It animates in with a scale + opacity transition. Clicking the overlay or pressing Escape closes the modal. Focus should be trapped inside the modal while it's open.
...
**EXAMPLES:**
- Radix UI Dialog — radix-ui.com/primitives/docs/components/dialog
- Flowbite Modal Examples — flowbite.com/docs/components/modal
- Dribbble Modal Design — dribbble.com/tags/modal

#### 6.6 Card Component
**WHAT_IT_IS:** A card is a self-contained rectangular container that groups related information about a single subject — a blog post, a product, a team member, a project. Cards are the most universal building block of web UI design. They create visual separation between items without requiring hard dividers, and they make complex pages scannable because each card is its own focused unit of content.
**HOW_IT_WORKS:** A basic card has: a visual area at the top (image, illustration, or colored block), a content area below with a title, description, optional metadata (date, author, tags, reading time), and sometimes a call-to-action (link, button). Cards are grouped in grids (2, 3, or 4 columns) and often have a hover state — a subtle lift effect (`translateY(-4px)` with increased shadow) that signals interactivity.
**KEY_TERMS:**
- Visual area — Top section: image, video thumbnail, illustration, or solid color block
- Content area — Padded section below: title, description, meta info, CTA
- Hover lift — translateY(-4px) + increased box-shadow on hover
- overflow:hidden — Applied to the card container, clips the image to the card's border-radius
- aspect-ratio — Keeps cards consistent height regardless of image
**TIP:** Three things make or break a card grid: consistent image aspect ratios (use aspect-ratio:16/9 or 4/3 on the image container), comfortable internal padding (20-24px is standard), and a clear hover state. Without a hover state, cards feel static and users don't know they're clickable.
...
- Tailwind UI Card Components — tailwindui.com/components/marketing/elements/cards
- Read.cv — read.cv

---

## QUICK REFERENCE VOCABULARY (render on root index)

Render as a 2-column responsive grid of definition tiles.
...
- [ ] All 39 sub-pages exist at the exact paths above.
- [ ] Each sub-page has a working live demo of its technique.
- [ ] Each sub-page has a "Copy Prompt" button that copies the exact prompt text to clipboard.
- [ ] Sticky navbar with breadcrumbs and theme toggle works on every page.
- [ ] Prev/Next pager links work and follow chapter order.
- [ ] Site is responsive at 480 / 768 / 1024 / 1280 breakpoints.
- [ ] No build step is required — open `index.html` directly in a browser.
- [ ] Dark/light theme persists via localStorage.
- [ ] External example-site links open in a new tab (`target="_blank" rel="noopener"`).
- [ ] All CDN-dependent demos (GSAP, Lenis, SplitType) gracefully degrade if CDN is offline.
